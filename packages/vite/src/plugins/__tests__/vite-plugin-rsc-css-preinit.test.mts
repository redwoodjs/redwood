...
test('generateCssMapping: correctly generates css mapping', () => {
  const mapping = generateCssMapping(clientBuildManifest)

  /**
   * In this snapshot we are looking for the following:
   * - the counter components has the css mapping: components/Counter/Counter.tsx
   * - the subcounter component has the css mapping: components/DeepSubCounter/DeepSubCounter.tsx
   * - entry client has the css mapping: entry.client.tsx
   * - generates a mapping for css modules: _Counter-IM_HERE_BECAUSE_OF_CSS_MODULES.mjs
   *
   */
...
test('generateServerComponentClientComponentMapping: correctly generates server to client component mapping', () => {
