...
export default function redwoodPluginVite(): PluginOption[] {
  ...
  return [
    ...
    },
    {
      ...
      // @MARK: Using the config hook here let's us merge the config with defaults
      // but returning plugins will **not** work, see https://vitejs.dev/guide/api-plugin.html#config
    ...
    // @MARK: Removed React plugin from this rw-vite plugin
    // Because we need to configure it conditionally based on whether
    // we are building for SPA/SSR or for RSC
    // There must be a better way to do this, but I can't figure it out right now
  ]
}
