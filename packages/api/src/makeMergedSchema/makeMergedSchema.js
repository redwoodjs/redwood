...
import { mergeSchemas, makeExecutableSchema, addResolveFunctionsToSchema } from 'apollo-server-lambda'
...
const mergeResolversWithServices = ({ schema, resolvers, services }) => {
  const mergedServices = merge({}, ...Object.keys(services).map(name => services[name]))
  return {
    ...resolvers,
...
export const makeMergedSchema = ({ schemas, services }) => {
  // Combine all the schemas together.
  ...
  const resolvers = mergeResolversWithServices({ schema, resolvers: mergeResolvers(schemas), services })
