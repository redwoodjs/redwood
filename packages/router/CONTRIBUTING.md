...
- Type / conver to TypeScript; in progress at https://github.com/redwoodjs/redwood/issues/791.
- Fix layout rerendering / optimize rerendering in general. Relevant issue: https://github.com/redwoodjs/redwood/issues/267.
- [Prerendering](https://community.redwoodjs.com/t/prerender-proposal/849)
...
The way history works (or, perhaps, the way _we_ work with it) is the thing that's changed the most. Today, we load new content dynamically, without loading a new document. But we still want navigation to be intuitive&mdash;back should send you back, forward should send you forward. So the question becomes: if we're doing it all dynamically, how does the browser know where to send us?

The TLDR is basically now we have to do it all ourselves. 

> In the we-have-to-do-it-all-ourselves sense, you can kind of think of client-side routing like GraphQL: you write the schema, you write the resolvers. There's no part of it that's done for you (aside from whatever auto-generating library you or your framework uses). Same with client-side routing: you `pushState` the new state, title, and url (~schema), and then have to get there somehow (~resolvers).

We have to do it all ourselves, but that doesn't mean we don't have APIs to work with. Client-side routing uses the `location` and `history` Web APIs, along with the `popstate` window event. Let's go over each.

#### Location

[`Location`](https://developer.mozilla.org/en-US/docs/Web/API/Location) represents the location (i.e. url) of the document in the window. It's pretty straightforward. The properties we use the most are `pathname`, `hash`, `search`. Using a url as an example:
...
Assuming you're on https://redwoodjs/com, if you run the example in the console, you should see the url change: it should be http://redwoodjs.com/bar.html. But notice that the browser didn't actually do anything&mdash;it didn't load `bar.html`, or even check that it exists. What gives?

Although the `location` updates (and that's important to remember), `pushState` doesn't do actually do anything to the document per se. It just an API for adding entries to the history stack. The three parameters it takes are: a state object (which you won't really see us using), a title (which you also won't really see us using), and a URL (we use this!).
...
So when we `navigate`, location will change from the call to `pushState` (`<Link>`'s `to` prop gets passed as the argument for `pushState`'s `url`). Then each listener gets re-run. This is where the "resolver" part of our router starts. The question now is what is in our `listeners` array? [In `location.js`](https://github.com/redwoodjs/redwood/blob/1e3cfcbf67b6cdca2e0dfbba000bcb67a7204f50/packages/router/src/location.js#L19-L23), we set up a listener that updates the `<LocationProvider>` component's state:
...
So every time we navigate, the `LocationProvider` gets the updated location. As the name would suggest, the LocationProvider is providing context, so anything that subscribes to it will get the news. But also, whenever its state updates, it's going to rerender. And that means the Router will too:
...
The router will look for a page match and render that page. That's basically how it works!
