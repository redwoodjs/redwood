...
Redwood uses [Babel](https://babeljs.io/) so that you can write modern JavaScript and TypeScript without needing to worry about transpilation at all.
GraphQL tags, JSX, SVG imports—all of it's handled for you.

> For those well-versed in Babel config, you can find Redwood's in the [`@redwoodjs/internal`](https://github.com/redwoodjs/redwood/tree/main/packages/internal/src/build/babel) package.

### Configuring Babel

For most projects, you won't need to configure Babel at all.
But if you need to, you can configure each side (web, api) individually using side-specific `babel.config.js` files.

> **Heads up**
>
> `.babelrc{.js}` files are ignored.
> You have to put custom config in the appropriate side's `babel.config.js`: `web/babel.config.js` for web and `api/babel.config.js` for api.
...
At the root of your project is a file named `jest.config.js`.
