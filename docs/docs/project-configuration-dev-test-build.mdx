...
Add/change the `apiDebugPort` under your api settings
...
[api]
  port = 8911
  // highlight-next-line
  apiDebugPort = 18911 # 👈 change me!
```

If you set it to `false`, no debug port will be exposed. The `apiDebugPort` is only ever used during development when running `yarn rw dev`
...
```bash
yarn rw dev --apiDebugPort 75028
```
