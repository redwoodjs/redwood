...
Love **[Redwood](https://redwoodjs.com)** and want to get involved? You’re in the right place!


> Before interacting with the **Redwood** community, please make sure you've read and understood our [Code of Conduct](https://github.com/redwoodjs/redwood/blob/main/CODE_OF_CONDUCT.md).
...
- [Contributing](#contributing)
  - [Local Development](#local-development)
    ...
    - [Testing Framework code in your App](#testing-framework-code-in-your-app)
      - [Option 1: Linking](#option-1-linking)
        - [Flags for `rwt link` command:](#flags-for-rwt-link-command)
  ...
  - [CLI Reference: `redwood-tools` (`rwt`)](#cli-reference-redwood-tools-rwt)
      - [`copy` (`cp`)](#copy-cp)
...
>:bell: If you already have a local copy, make sure you've got the `main` branch's latest changes with `git pull`.
...
**Using the functional test project might be the easiest and fastest way to test your changes in Redwood**.

You can create a **Redwood** project that contains a big range of functionality in a few minutes:
...
> **PRO TIP**: You can avoid having to provide the path to `redwood` by defining an `RW_PATH` environment variable on your system. See [Specifying a `RW_PATH`](#specifying-a-rw_path).
...
you can open a new tab in your terminal, *cd* to `redwood-app` and launch your **Redwood App** from
there. Ten, as you make changes in `redwood`, you will see those changes immediately in the
behavior of your **Redwood App**.
...
Then, you can restore your **Redwood App** to its original state by deleting `./node_modules`,
`web/node_modules`, and `api/node_modules`, and then running `yarn install`.

#### Option 3: Copy (for Windows)

If you are on **Windows** and not using **WSL**, you will have to use `rwt cp`
> This approach is tracked in [issue #701](https://github.com/redwoodjs/redwood/issues/701), and,
> currently, unfortunately, it will not let you see your changes live.
>
...
> **PRO tip**: if not using `bash` adapt steps bellow to fit the needs of your shell of choice.
>
##### On **Linux**
...
Then, in your **Redwood App**, you can just run:
...
Sometimes you'll want to test the full package development workflow &mdash; building, publishing, and installing all the packages in your local copy of the **Redwood Framework** in your **Redwood App**. We accommodate this using a local **NPM** registry called [**Verdaccio**](https://github.com/verdaccio/verdaccio).
...
## CLI Reference: `redwood-tools` (`rwt`)

**Redwood**'s companion CLI development tool. You'll be using this if you're contributing to
**Redwood**.
...
> **PRO tip**: You can avoid having to provide `RW_PATH` by defining an environment variable on your system. See [Specifying a `RW_PATH`](#specifying-a-rw_path).
